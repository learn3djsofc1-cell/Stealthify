    ...
    <div className="space-y-6 sm:space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/60">View and manage your stealth browsing sessions</p>
        </div>
        <Button
          variant="primary"
      ...
      {sessions.length === 0 ? (
        ...
        <>
          {activeSessions.length > 0 && (
            <div>
              <h2 className="text-xs font-medium text-emerald-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                  ...
                  <Card key={session.id} glow="rgba(16, 185, 129, 0.06)" hover onClick={() => navigate(`/app/session/${session.id}`)}>
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-white/50 mt-0.5 truncate font-mono">{extractDomain(session.target_url)}</p>
                      </div>
                      <div className="hidden sm:flex items-center gap-3 shrink-0">
                        <div className="text-right">
                          <p className="text-xs text-white/50">Duration</p>
                          <p className="text-xs text-white/60 font-mono">{formatDuration(session.started_at, null)}</p>
                        </div>
                        <button
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/[0.08]">
                        <th className="px-4 sm:px-6 py-3.5 text-left text-[11px] font-medium text-white/50 uppercase tracking-wider">
                          <div className="flex items-center gap-1.5">
                            <Globe className="h-3 w-3" />
                            Target
                          </div>
                        </th>
                        <th className="px-4 sm:px-6 py-3.5 text-left text-[11px] font-medium text-white/50 uppercase tracking-wider">Status</th>
                        <th className="hidden sm:table-cell px-6 py-3.5 text-left text-[11px] font-medium text-white/50 uppercase tracking-wider">
                          <div className="flex items-center gap-1.5">
                            <Clock className="h-3 w-3" />
                            Duration
                          </div>
                        </th>
                        <th className="hidden md:table-cell px-6 py-3.5 text-left text-[11px] font-medium text-white/50 uppercase tracking-wider">
                          <div className="flex items-center gap-1.5">
                            <ShieldCheck className="h-3 w-3" />
                            Privacy
                          </div>
                        </th>
                        <th className="px-4 sm:px-6 py-3.5 text-right text-[11px] font-medium text-white/50 uppercase tracking-wider">When</th>
                      </tr>
                    </thead>
                    <tbody>
                      {pastSessions.map((session) => (
                        <tr
                          key={session.id}
                          className="border-b border-white/[0.06] last:border-0 hover:bg-white/[0.04] cursor-pointer transition-colors"
                        ...
                        >
                          <td className="px-4 sm:px-6 py-4">
                            <div>
                              ...
                              <p className="text-xs text-white/45 font-mono truncate max-w-[200px]">{extractDomain(session.target_url)}</p>
                            </div>
                          </td>
                          <td className="px-4 sm:px-6 py-4">
                            <Badge
                          ...
                          <td className="hidden sm:table-cell px-6 py-4 text-sm text-white/55 font-mono">
                          ...
                          <td className="px-4 sm:px-6 py-4 text-right">
                            <span className="text-xs text-white/50">{timeAgo(session.started_at)}</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
