...
const Hero = () => {
  ...
  const isInView = useInView(ref, { margin: "0px 0px -200px 0px" }); // Unmount when scrolled out
    ...
    <div ref={ref} className="relative w-full min-h-screen bg-[#050505] overflow-hidden flex flex-col font-sans selection:bg-purple-500/30">
      ...
      <main className="flex-grow flex flex-col lg:flex-row items-center justify-between relative max-w-7xl mx-auto w-full px-6 pt-24 lg:pt-0 z-10">
        ...
        <div className="w-full lg:w-1/2 z-20 flex flex-col justify-center lg:pr-12 mt-12 lg:mt-0">
          ...
          >
            <button className="px-6 py-3 bg-white text-black rounded-full font-medium text-xs flex items-center gap-2 hover:bg-gray-200 transition-colors duration-300">
              Launch App
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
            <button className="px-6 py-3 bg-[#1A1A1A] border border-white/10 text-white rounded-full font-medium text-xs hover:bg-[#252525] hover:border-white/20 transition-all duration-300">
              Learn More →
            </button>
          </motion.div>
        </div>
